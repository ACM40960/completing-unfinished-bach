...
The full printout (and per-voice SATB extraction utilities) can be reproduced with the exploration cells in [Notebooks/01_data_extraction.ipynb](Notebooks/01_data_extraction.ipynb).
...
![Chord transition probabilities](images/Markov%20hmap.png)
...
Full classification reports are generated in [Notebooks/04_chained_model_evaluation.ipynb](Notebooks/04_chained_model_evaluation.ipynb).
...
> **Note:** the rendered `.wav` files (Bach vs the model) are not stored in the repository — they are generated by [Notebooks/05_audio_rendering.ipynb](Notebooks/05_audio_rendering.ipynb). You can listen to them directly here: **[Bach vs the model — audio files](https://jumpshare.com/folder/54VetyiCjVccDgBnDJ0D)** (the poster's QR code links to the same demo).
...
├── Notebooks/
│   ├── 01_data_extraction.ipynb            # Corpus search, duplicate removal, Roman numeral dataset
│   ├── 02_markov_analysis.ipynb            # Transition heatmaps, Markov cross-entropy baseline
│   ├── 03_model_sweeps.ipynb               # Linear / GRU / LSTM hyperparameter sweeps
│   ├── 04_chained_model_evaluation.ipynb   # Chained multitask LSTM, tau, confusion matrices
│   └── 05_audio_rendering.ipynb            # Bach vs model rendered to MIDI and .wav
